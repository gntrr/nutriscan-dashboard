...
export const POST: APIRoute = async ({ request, locals, redirect }) => {
  ...
  try {
    ...
    // Approve nakes account
    const { error } = await supabase
      .from('nakes_accounts')
      .update({
        is_active: true,
        approved_at: new Date().toISOString(),
        approved_by: nakes.id,
        updated_at: new Date().toISOString(),
      })
      .eq('id', nakesId);

    if (error) {
      console.error('Error approving nakes:', error);
      return redirect('/dashboard/admin/nakes?error=approval_failed');
    }

    return redirect('/dashboard/admin/nakes?success=approved');
  } catch (error) {
